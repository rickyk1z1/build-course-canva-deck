...
`build-course-canva-deck` 是一个 Codex Skill，用于把 PDF、XMind、DOCX、Markdown、TXT、OPML、FreeMind 等课程大纲制作成适合线上录课的 Canva 演示课件。讲稿只能在同时提供权威大纲时作为屏显文案参考，不能作为权威来源。
...
当前 skill 使用一个总导演加四个 proposal-only 子代理。总导演先读取完整上下文，再给每个子代理下发裁剪后的任务包。子代理只读任务包允许的文件或摘录，只写 scratch 提案，不写最终文件、不改 Canva、不改源文件。
...
- `课堂编剧`：基于已批准的节点摘录和保真计划写可独立理解的屏显文案；如果用户同时提供讲稿，只把它当作措辞和教学重心参考。
- `视觉分镜师`：分阶段规划视觉；先做源图复用、视觉类型、生图候选和容量风险 triage，再在文案稳定后补模板视觉语言、版式容量和已批准的生图任务。

子代理工作过程默认使用中文；JSON key、脚本参数、文件路径、schema enum 和工具标识保持英文。

子代理名称代表同一个逻辑角色，不代表每次调用都新建一个独立身份。重复调用 `课程统筹师`、`原稿场记`、`课堂编剧` 或 `视觉分镜师` 时，总导演在下一份 brief 中重述上一轮已批准的状态、提案和未解决问题，让后续调用连续推进而不自相矛盾；即使运行环境物理上开启了新子代理，也当作同一逻辑角色的连续调用。没有 per-role 状态文件。

详细职责见 `skills/build-course-canva-deck/references/roles-and-delivery.md`。
...
Skill 不会根据文件长度、层级深度、是否像讲稿或主观判断自动选择模式。

如果用户同时给到 XMind/大纲和讲稿，XMind/大纲仍然是唯一权威来源，仍只在 `细纲` / `粗纲` 中选择。讲稿可记录为 `course.reference_script`，用途固定为 `screen-copy-reference-only`，只帮助屏显文字更贴合讲课表达，不参与章节、覆盖、源图锚点、生图任务或顺序判断。
...
- 学员页面不得出现 `PDF`、`原稿`、`制作说明`、`详细讲稿`、`根据讲稿整理`、`这一段讲的是` 等幕后措辞。
- 脚本只拦确定性硬伤。最终是否像真人讲师课件，由总导演按学员视角审查：层级顺、逻辑清、标签有意义、视觉能教学、版面不遮挡。

## 视觉和 Canva 交付

- 每个普通知识页都要有教学型视觉或明确的 text-only 例外：源案例图、重绘源图、生成案例图、可编辑图解或表格。
- 优先复用源案例图；源图都保留后，对缺图但需要视觉案例的文字页或抽象页再补无内嵌中文的生成图。生成图不按数量凑，源图和可编辑图解足够时必须记录跳过生图的具体理由。
- 视觉分镜师可按阶段或连续页段输出视觉提案，只规划已批准的 `image_generation_tasks`；总导演负责调用生图工具、保存资产、写入 spec 和构建课件。
