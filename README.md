...
当前 skill 使用一个总控 Agent 加五个 proposal-only 子 Agent。子 Agent 只写 scratch 提案，不写最终文件、不改 Canva、不改源文件。
...
- `Controller Agent`：唯一负责最终写入、合并、构建 PPTX、导入 Canva 和交付。
- `Source & Curriculum Agent`：确认课程位置、邻课边界、术语和排除主题。
- `Source Fidelity Agent`：解析源大纲顺序、节点分组、同级枚举、源图和保真风险。
- `Learning Copy Agent`：写可独立理解的屏显文案和每个源节点的可见证据。
- `Visual Layout Agent`：规划案例图、生成图、模板页、Canva-native motif 和版式容量。
- `Quality Gate Agent`：只审查，不创作；负责提案边界、顺序、重复、缺失和渲染后 QA。
