#!/usr/bin/env python3
"""Regression test: non-thumbnail source images cannot be silently omitted."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        source = {
            ...
            "images": [
                ...
                {"id": "img003", "path": "/tmp/thumbnail.png"},
            ],
        }
        deck = {
            "course": {
                ...
                "source_image_coverage": [
                    ...
                    {"source_image_id": "img002", "status": "omitted", "reason": "bad old behavior"},
                    {"source_image_id": "img003", "status": "thumbnail_omitted"},
                ],
            },
            "slides": [
                {
        ...
        if not any("img002" in err and "must be used or redrawn" in err for err in report["errors"]):
            raise AssertionError(report)
        if not any("missing 1 source image" in err or "missing 1 source image(s)" in err for err in report["errors"]):
            raise AssertionError(report)
    print("source image audit regression passed")
    return 0
