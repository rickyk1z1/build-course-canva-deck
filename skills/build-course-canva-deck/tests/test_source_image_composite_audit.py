#!/usr/bin/env python3
"""Regression test: multiple source images cannot be hidden in one tiny composite."""
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        source = {
            ...
            "images": [
                {"id": "img001", "path": "/tmp/case-a.png", "source_node_id": "n3", "source_node_text": "point"},
                {"id": "img002", "path": "/tmp/case-b.png", "source_node_id": "n3", "source_node_text": "point"},
            ],
        }
        deck = {
            "course": {
                ...
                "source_image_coverage": [
                    {"source_image_id": "img001", "status": "used"},
                    {"source_image_id": "img002", "status": "used"},
                ],
            },
            "slides": [
                {
                ...
                },
                {
                    ...
                    "screen": {
                        ...
                        "caption": "inspect both images",
                        "blocks": [],
                    },
                    "source_node_treatments": [
                        {"source_node_id": "n3", "coverage_status": "visualized", "screen_evidence": "point"}
                    ],
                    "scope_check": {"status": "within-branch", "branch_node_id": "n2"},
                    "visual_plan": {
                        ...
                        "source_node_id": "n3",
                        "source_image_ids": ["img001", "img002"],
                        "labels_as_slide_text": True,
                        "case_visual_map": [{"screen_evidence": "visible idea", "visible_detail": "case details"}],
                    },
                    "visuals": [{"path": "assets/composites/tiny-montage.png"}],
                    "added_content": [
                        {
                            "source_node_id": "n3",
                            "kind": "definition",
        ...
        if not any("composite/montage" in err for err in report["errors"]):
            raise AssertionError(report)
        if not any("separate large panels" in err for err in report["errors"]):
            raise AssertionError(report)
    print("source image composite audit regression passed")
    return 0
