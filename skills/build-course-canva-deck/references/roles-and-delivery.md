...
1. 总导演 does source intake, extraction, mode recording, curriculum discovery, full source-structure reading, approved `chapter_spine`, and source-image inventory with preserved `source_node_id` / `source_path` anchors for every non-thumbnail source image.
...
6. 视觉分镜师 finalizes structural layout families, complete non-thumbnail source-image placement by extracted source-node anchors, per-slide no-source visual decisions, case-image stage sizing for source/redrawn/generated case images, visuals, layout capacity, template references, case-visual maps, section-internal layout variety, composition balance, and `image_generation_tasks`. Long decks may split by contiguous slide ranges. The final visual pass must list every normal knowledge page with no source image and say one of: `generated-image task created`, `editable-diagram/table is clearer because...`, or `text-only exception because...`.
