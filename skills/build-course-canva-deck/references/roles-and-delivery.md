...
1. 总导演 does source intake, extraction, mode recording (`细纲` / `粗纲` / `讲稿`), curriculum discovery, full source-structure or script-sequence reading, approved `chapter_spine`, and source-image inventory with preserved `source_node_id` / `source_path` anchors for every non-thumbnail source image.
