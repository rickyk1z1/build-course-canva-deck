...
- Plan each page from a reference template page or page family before generating the local PPTX. The slide spec must record `visual_plan.template_reference` with the inherited layout features and adaptation decision. Do not build a generic rectangular layout first and then add template-looking decoration later.
