...
- For long decks, also record `visual_plan.rendered_pattern` or `visual_plan.thumbnail_pattern` in plain language: what the contact sheet will visibly look like at thumbnail size, such as `dark-image-evidence-split`, `full-field-branch-map`, `compact-index-grid`, `accent-statement-visual-band`, or `bottom-dual-card-strip`. This field is a design promise, not a way to rename repeated pages. If two pages have different variants but the same visible geometry, they are repeated.
...
- `build_deck.mjs` must also use `visual_plan.rendered_pattern` or `thumbnail_pattern` as an execution hint for actual geometry. Text-led pages should have distinct renderer paths for flow/roadmap nodes, statement fields, side rails/modules, dense grids, and summary structures instead of falling back to the same two-card information strip.
- The director must compare `rendered_pattern` against the actual contact sheet. A mismatch is a failed build-plan loop, not a field naming problem.
...
- Any repeated-layout failure should trigger a review of template atlas, page mapping, screen-copy density, visual role, and renderer support together. Do not repair it by only changing colors or adding new variant names.
