...
- For long decks, plan enough native motif usage to be visible across the deck: at least four slides or roughly 20% of normal knowledge slides, whichever is larger when the selected template has reusable assets. If the connector cannot insert a native asset, record the blocker and stop before final delivery rather than silently leaving a local proxy.
