...
2. **章节封面提前讲正文** — 每张章节封面的小字是否只是短 agenda cue?有没有把后面普通页的标题、证据短语、快捷键串、操作清单、完整知识点或“这一节要解决什么”“正确做法是什么”这类结论/承诺提前放上来?章节封面可以锚定子节点,但不能把子节点正文 copy 原样搬上去。
...
15. **无源页连续绕过生图** — 看每一节内部:有没有连续 3 页以上普通知识页都没有源图/生图,只用文字、表格或可编辑图解并靠 bypass reason 过关?有没有一张 generated-image 被当成整套 deck 的通行证,后面无关页面继续不生图?这类情况要回到视觉分镜,在相关知识点附近补 generated/source/redrawn case-image 页或重拆 section。
16. **生成图离题** — 每张生成图,学员能否在三秒内看出它教的是哪个知识点?只是漂亮、符合配色、通用场景的图要换掉。
17. **拥挤/截断** — 有没有页面文字溢出、被压字、出现 `…`/`...` 截断,或为塞下内容用了 12-13pt?看到省略号不是“压缩成功”,而是必须拆页或重写。
18. **字号过小** — 正文是否 <16pt、标题是否 <36pt?录课时要看得清。
19. **版面失衡/重叠** — 有没有像内容都挤在上半区、下半区空掉、主块和辅助块比例失调、文字与元素重叠的页面?这类页即使没有脚本 overflow 也失败。
20. **留白和边界失控** — 看 full-size 页面:图片、文字、图解是否意外贴住白/黑/橙色块边缘或页面边缘?案例图是否像被硬贴进矩形,而不是有意识地装在共享舞台里?如果整页只有一张源图,这张图是否真的能视觉支撑共享舞台,还是原图太小/太稀疏导致大面积空白?这种情况下是否补了贴近图像的解释文字、判断点,或放入同锚点的第二张图,而不是继续让小图孤零零浮在大白框里?多图同页时,每张图之间和图到舞台边缘是否有稳定、可读、符合模板气质的呼吸空间?不要只看脚本是否通过,要按平面设计常识判断对齐轴、内边距和负空间是否舒服。
21. **多图比例被机械横排** — 多张案例图同页时,是否不管横图/竖图/界面长条比例都硬做等宽横排?如果图片比例、信息密度或教学主次不同,应使用主次大小、错落、竖向堆叠或上下/侧边组合；只有比例相近且教学关系确实平行时,才使用等分横排。
22. **模板理解偏差** — 整套是否只是"套了模板配色的白底通用页",或错误理解成所有元素都要方方正正的块?重点看是否贴近模板的整体布局、对齐轴、色块比例、字号梯度、图片处理和留白。
23. **课程越界** — 有没有抢讲相邻课的主任务,或重复了其它课已完整覆盖的内容?
