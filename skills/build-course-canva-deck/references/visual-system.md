...
Generated case images usually fail for four human reasons: the prompt starts from mood or palette instead of the point being taught; the scene is generic and could fit any slide; the learner cannot see a decision, contrast, consequence, or misconception in the image; or the director accepts the image at asset size without checking it inside the slide. Fix those causes before changing prompt wording.
...
- `instant_takeaway`: the one-sentence understanding a zero-basis learner should get from the image before the instructor explains it;
...
If the visual planner cannot name a concrete `observable_teaching_detail` and `instant_takeaway`, do not generate an image yet. Choose an editable diagram, split/rewrite the page, or ask for a clearer teaching example. After generation, inspect the image at slide size before PPTX build. Regenerate or replace it when the picture looks stylish but the learner cannot tell which exact knowledge point it teaches.

Prefer vivid teaching scenes over generic illustrations:

- Show a before/after contrast, wrong/right choice, visible consequence, physical analogy, or concrete workflow artifact.
- Make one thing in the image unmistakably carry the point. Do not rely on the caption to explain why a vague scene belongs.
- Avoid a generic person at a desk, floating icons, abstract arrows, decorative dashboards, or mood-only studio shots unless the slide's source node specifically teaches that object.
- Keep template color and lighting aligned enough that the image belongs in the deck, but never let palette matching replace teaching specificity.
...
- Record the executable task list in `course.image_generation_tasks` when generated illustrations are planned. Each task must preserve the `knowledge_anchor`, `observable_teaching_detail`, and `instant_takeaway`; prompt wording can vary, but those teaching constraints cannot disappear.
...
Before building the PPTX, inspect generated images at slide size and ask: "What visible detail in this exact image teaches this slide's knowledge point, and what should a zero-basis learner understand within three seconds?" If the answer is only mood, style, color, or a generic scene, regenerate or replace it before Canva import.
