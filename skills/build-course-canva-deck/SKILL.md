...
- Planned Canva-native elements must be real delivery requirements, not placeholders. `visual_plan.template_motif.canva_asset_id` must name the native asset to use after import, and final delivery is blocked if proxy or pending motif IDs remain unverified.
